...
const eventObject = { target: { name: 'newPoint', value: 'abc' } }
...
describe('rendering new point form component with error input', () => {
    beforeEach(() => {
        ...
        act(() => {
            component = create(<NewPointForm addNewPoint={mockCallback} isNoPointInCenter={false} />)
        })
    ...
    test(`there is should be error <div> message if center coordinates wasn't updated`, () => {
        const div = componentRoot.findByProps({ className: 'newPointForm__errorText' })
    ...
    test(`component shouldn't call callback if input value is empty`, () => {
        const eventObject = { target: { name: 'newPoint', value: '' } }
        act(() => {
            component = create(<NewPointForm addNewPoint={mockCallback} isNoPointInCenter={true} />)
        })
        componentRoot = component.root
        input = componentRoot.findByType('input')
