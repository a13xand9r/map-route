...
describe('route points list component', () => {
    test(`dispatch should be called after onDragEnd`, () => {
        const action = { destIndex: 2, sourceIndex: 0, type: 'CHANGE_POINTS_ORDER' }
        const dragDrop = componentRoot.findByType(DragDropContext)
    ...
    test(`dispatch should be called after adding a new point`, () => {
        ...
        const newPointForm = componentRoot.findByType(NewPointForm)
    ...
    test(`dispatch for point deleting should be called`, () => {
        const action = { type: 'DELETE_POINT', id: 1 }
        const pointItem = componentRoot.findAllByType(PointItem)[0]
        const deleteButton = pointItem.findByType('span')
